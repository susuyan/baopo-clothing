    ...
    <div className="md:max-w-7xl md:mx-auto md:px-6 md:py-8">
      ...
      <div className="md:hidden sticky top-0 z-40 bg-white border-b border-gray-100 px-3 pt-2.5 pb-3">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`shrink-0 px-3 py-2 text-xs font-medium rounded-full border transition-colors ${
              sortBy !== "default"
      ...
      <div className="hidden md:flex items-center gap-3 mb-6">
        <div className="flex-1 relative">
          ...
          <input
            ...
            onChange={(e) => setQuery(e.target.value)}
            placeholder="搜索货号或关键词..."
            className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500"
          />
        </div>
        <select
          value={categoryId}
      ...
      <div className="px-3 md:px-0 py-2 md:py-0">
        <p className="text-xs text-gray-400">
          共 {filteredProducts.length} 款 {query && `· "${query}"`} {categoryId !== "all" && `· ${categories.find(c => c.id === categoryId)?.name}`}
        </p>
      </div>
      ...
      <div className="md:hidden grid grid-cols-2 gap-2 px-3 pt-2 pb-20">
        {filteredProducts.map((p) => (
          <Link
            key={p.id}
            href={`/products/${p.id}`}
            className="block bg-white rounded-xl overflow-hidden active:scale-[0.97] transition-transform"
          >
            <div className="relative aspect-[3/4] bg-gray-100">
              <Image
                ...
                loading="lazy"
              />
              <span className="absolute top-1.5 left-1.5 px-1.5 py-0.5 bg-black/60 text-white text-[9px] rounded">
                {p.category_name}
              </span>
            </div>
            <div className="p-2.5">
